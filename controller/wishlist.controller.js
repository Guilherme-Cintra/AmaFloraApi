...
export const createFav = async (req, res) => {
  ...
  export const deleteFav = async (req, res) => {
    try {
      ...
      const fav = await WishList.findByPk(id);
      if (fav) {
        await fav.destroy();
      ...
      // En cas d'erreur, renvoie une réponse d'erreur
      res.status(500).json({ message: "Erreur lors  de la suppression du favorit", error });
    }
  };
  ...
  export const getWishList = async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Erreur dans la recherche de la liste de souhaits", error });
    }
  };